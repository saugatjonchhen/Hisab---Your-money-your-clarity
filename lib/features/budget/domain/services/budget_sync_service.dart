  ...
  static List<CategoryModel> calculateUpdatedLimits({
    ...
    for (var category in categories) {
      ...
      if (category.type == 'expense') {
        ...
        String? matchedKey;
        if ((name.contains('emi') || name.contains('loan'))) matchedKey = 'emi';
        else if ((name.contains('rent') || name.contains('house'))) matchedKey = 'rent';
        else if (name.contains('utility') || name.contains('bill')) matchedKey = 'utilities';
        else if (name.contains('education') || name.contains('school')) matchedKey = 'education';

        if (matchedKey != null && questionnaire.fixedExpenses.containsKey(matchedKey)) {
           ...
           if (name.contains('food') || name.contains('grocery') || name.contains('eat')) weight = 30;
           else if (name.contains('transport') || name.contains('fuel') || name.contains('auto') || name.contains('cab')) weight = 15;
           else if (name.contains('health') || name.contains('medical') || name.contains('doc') || name.contains('pharm')) weight = 10;
           else if (name.contains('shop') || name.contains('mall') || name.contains('cloth') || name.contains('item')) weight = 15;
           else if (name.contains('ent') || name.contains('fun') || name.contains('movie') || name.contains('sub') || name.contains('ott')) weight = 10;
           else if (name.contains('bill') || name.contains('utility') || name.contains('recharge')) weight = 10;
        ...
        if (cName.contains('save') || cName.contains('depo') || cName.contains('fd') || cName.contains('emergency') || cName.contains('piggy')) weight = 60;
        else if (cName.contains('stock') || cName.contains('equity') || cName.contains('mutual') || cName.contains('fund') || cName.contains('invest') || cName.contains('crypto') || cName.contains('gold') || cName.contains('sip')) weight = 40;
        else if (category.type == 'savings') weight = 30; // Generic fallback
        else weight = 20; // Generic investment fallback
