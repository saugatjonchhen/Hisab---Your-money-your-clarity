...
class _DetailedStatsPageState extends ConsumerState<DetailedStatsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        padding: const EdgeInsets.fromLTRB(AppValues.horizontalPadding, 0,
            AppValues.horizontalPadding, AppValues.gapExtraLarge),
        child: Column(
          children: [
            ...
            transactionsAsync.when(
              data: (transactions) {
                ...
                final previousTransactions = _compareMode
                    ? _filterPreviousTransactions(transactions)
                    : <TransactionModel>[];
                  ...
                  children: [
                    SpendingChart(
                      ...
                      getBottomTitles: (value, meta) =>
                          _getBottomTitles(value, meta, _viewMode),
                      maxX: maxX,
                    ),
                    const SizedBox(height: AppValues.gapLarge),
  ...
  List<TransactionModel> _filterPreviousTransactions(
      List<TransactionModel> transactions) {
    final now = DateTime.now();
    final yesterday = now.subtract(const Duration(days: 1));
    ...
    if (_viewMode == DashboardViewMode.weekly) {
      ...
      final endDate =
          DateUtils.dateOnly(now).subtract(const Duration(days: 30));
      final startDate = endDate.subtract(const Duration(days: 29));
      return transactions.where((t) {
        final tDate = DateUtils.dateOnly(t.date);
        return tDate.isAfter(startDate.subtract(const Duration(days: 1))) &&
  ...
  List<BarChartGroupData> _generateBarGroups(
    ...
    for (int i = 0; i < daysCount; i++) {
      ...
      if (_compareMode) {
        ...
        rods = [
          if ((income[i] ?? 0) > 0)
            _makeRodData(income[i]!, AppColors.secondary),
          if ((expense[i] ?? 0) > 0)
            _makeRodData(expense[i]!, AppColors.tertiary),
          if ((savings[i] ?? 0) > 0)
            _makeRodData(savings[i]!, AppColors.savings),
          if ((investment[i] ?? 0) > 0)
            _makeRodData(investment[i]!, AppColors.investment),
        ];
      }
  ...
  Widget _buildStatsSummary(
      List<TransactionModel> transactions, String currencySymbol) {
    ...
    return Column(
      children: [
        Row(
          children: [
            Expanded(
                child: _buildSummaryCard('Income', updatedIncome,
                    AppColors.secondary, currencySymbol)),
            const SizedBox(width: AppValues.gapMedium),
            Expanded(
                child: _buildSummaryCard('Expense', updatedExpense,
                    AppColors.tertiary, currencySymbol)),
          ],
        ),
        const SizedBox(height: AppValues.gapMedium),
        Row(
          children: [
            Expanded(
                child: _buildSummaryCard('Savings', updatedSavings,
                    AppColors.savings, currencySymbol)),
            const SizedBox(width: AppValues.gapMedium),
            Expanded(
                child: _buildSummaryCard('Investment', updatedInvestment,
  ...
  Widget _buildSummaryCard(
      String title, double amount, Color color, String currencySymbol) {
    return Container(
  ...
  Widget _getBottomTitles(
      double value, TitleMeta meta, DashboardViewMode mode) {
    ...
    if (index < 0 || (mode != DashboardViewMode.yearly && index >= daysCount)) {
                ...
                return Container(
                  ...
                  child: ListTile(
                    leading: Container(
                      ...
                      child: Icon(_getIcon(iconName, isIncome, t.type),
                          color: color),
                    ),
                    title: Text(
                        t.note.isEmpty
                    ...
                    subtitle: Text(
                        '${DateFormat('MMM d').format(t.date)} • ${DateFormat('h:mm a').format(t.date)}',
        ...
        return Column(
          ...
          children: [
            ...
            ...sortedEntries.map((e) {
              ...
              final percentage =
                  totalExpense > 0 ? (e.value / totalExpense) : 0.0;
              return Padding(
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(_getIcon(cat.iconParams, false, 'expense'),
                            size: 16, color: Color(cat.colorValue)),
                      ),
                      const SizedBox(width: AppValues.gapMedium),
                      Expanded(
                        child: Column(
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            final transactions =
                ref.read(transactionsListProvider).valueOrNull ?? [];
            final categories =
            ...
            if (_rangeMode == 'Weekly') {
              ...
              final startDate =
                  DateUtils.dateOnly(now).subtract(const Duration(days: 29));
              filtered = transactions
                  .where((t) => t.date
                      .isAfter(startDate.subtract(const Duration(days: 1))))
                  .toList();
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(
                    'Exporting ${filtered.length} transactions as $_format...')));
            final symbol = settings?.currencySymbol ?? r'$';
            final title = _rangeMode == 'Custom' && _customRange != null
                ? '${DateFormat('MMM d').format(_customRange!.start)}-${DateFormat('MMM d').format(_customRange!.end)}'
                : _rangeMode.toUpperCase();
            try {
