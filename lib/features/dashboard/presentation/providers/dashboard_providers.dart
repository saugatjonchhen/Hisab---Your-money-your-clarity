...
export 'package:finance_app/features/transactions/data/providers/transaction_provider.dart' 
  show totalWealthProvider, totalSavingsProvider, totalInvestmentProvider, totalBalanceProvider, totalIncomeProvider, totalExpenseProvider;
...
@riverpod
Future<List<TransactionModel>> filteredTransactions(FilteredTransactionsRef ref) async {
...
Future<BreakingDownSpending> spendingBreakdown(SpendingBreakdownRef ref) async {
  ...
  for (var t in transactions) {
     if (t.type == 'expense' && period.contains(t.date)) {
        final category = categories.firstWhere(
          (c) => c.id == t.categoryId, 
          orElse: () => CategoryModel(id: '', name: '', iconParams: '', colorValue: 0, type: '')
        );
  ...
  final plannedMandatory = (activePlan.allocations['Mandatory'] ?? 0) + (activePlan.allocations['Debt'] ?? 0);
  
  // 3. Calculate Remaining Reserved
  final remainingReserved = (plannedMandatory - committedSpent).clamp(0.0, double.infinity);
...
class BreakingDownSpending {
  ...
  BreakingDownSpending(this.totalReserved, this.reservedSpent, this.otherSpent, this.remainingReserved);
}

@riverpod
Future<double> remainingCommittedExpenses(RemainingCommittedExpensesRef ref) async {
...
Future<double> filteredBalance(FilteredBalanceRef ref) async {
  ...
  final double remainingCommitted = await ref.watch(remainingCommittedExpensesProvider.future);
...
@riverpod
Future<Map<String, double>> monthlyCategorySpending(MonthlyCategorySpendingRef ref) async {
  final transactions = await ref.watch(transactionsListProvider.future);
...
@riverpod
Future<List<CategoryBudgetProgress>> budgetProgress(BudgetProgressRef ref) async {
  final categories = await ref.watch(categoriesListProvider.future);
...
class TransactionTypeGroup {
  ...
  double get progress => totalLimit > 0 ? (totalSpent / totalLimit).clamp(0.0, 1.0) : 0.0;
  bool get isOverBudget => totalSpent > totalLimit;
}

@riverpod
Future<List<TransactionTypeGroup>> budgetProgressByType(BudgetProgressByTypeRef ref) async {
  final categories = await ref.watch(categoriesListProvider.future);
  final spending = await ref.watch(monthlyCategorySpendingProvider.future);
  ...
  for (var entry in groupedByType.entries) {
    ...
    final totalLimit = finalCategories.fold<double>(0.0, (sum, cat) => sum + cat.limit);
    final totalSpent = finalCategories.fold<double>(0.0, (sum, cat) => sum + cat.spent);
    ...
    if (mergedGroups.containsKey(type)) {
      ...
      mergedGroups[type] = TransactionTypeGroup(
        ...
        totalLimit: mergedUniqueCats.fold<double>(0.0, (sum, c) => sum + c.limit),
        totalSpent: mergedUniqueCats.fold<double>(0.0, (sum, c) => sum + c.spent),
      );
    } else {
      mergedGroups[type] = TransactionTypeGroup(
        type: type,
  ...
  if (activePlan != null && activePlan.id != 'empty') {
    ...
    for (var cat in categories) {
      if (cat.type == 'expense' || cat.type == 'Mandatory') {
        ...
        if (name.contains('emi') || name.contains('loan') || name.contains('rent') || 
            name.contains('house') || name.contains('bill') || name.contains('utility') || 
    ...
    final unallocated = (plannedMandatory - matchedMandatory).clamp(0.0, double.infinity);
    if (unallocated > 1.0) { // Only show if significant (> Rs. 1)
      ...
      final mandatoryKeywords = ['emi', 'loan', 'debt', 'repayment', 'mortgage', 'rent', 'utility', 'bill'];
      ...
      for (var t in transactions) {
        if (t.type == 'expense' && t.date.year == now.year && t.date.month == now.month) {
          ...
          final tCategory = categories.firstWhere((c) => c.id == t.categoryId, orElse: () => CategoryModel(id: '', name: '', iconParams: '', colorValue: 0, type: ''));
          ...
          if (!isExplicitlyBudgeted) {
            ...
            final isReservedMatch = isMandatoryKeyword || mandatoryKeywords.any((kw) => noteLower.contains(kw));
  ...
  final typePriority = {'expense': 0, 'reserved': 1, 'income': 2, 'savings': 3, 'investment': 4};
  groups.sort((a, b) => (typePriority[a.type] ?? 99).compareTo(typePriority[b.type] ?? 99));
